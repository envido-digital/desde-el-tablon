...
export function subscribe(email: string, name?: string, source = 'web'): {
  success: boolean;
  ...
  if (existing) {
    if (existing.status === 'active') return { success: true, alreadyExists: true };
    // Reactivate
    sqlite.prepare(
      "UPDATE newsletter_subscribers SET status='active', unsubbed_at=NULL WHERE id=?"
    ).run(existing.id);
    return { success: true };
  }
...
export function getActiveSubscriberCount(): number {
  return (sqlite.prepare(
    "SELECT COUNT(*) as c FROM newsletter_subscribers WHERE status='active' AND confirmed=1"
  ).get() as { c: number }).c;
}

function getActiveSubscribers(): Array<{ id: string; email: string; name: string | null; unsub_token: string }> {
  return sqlite.prepare(
    "SELECT id, email, name, unsub_token FROM newsletter_subscribers WHERE status='active' AND confirmed=1 ORDER BY created_at ASC"
  ).all() as Array<{ id: string; email: string; name: string | null; unsub_token: string }>;
}

// ─── Get best articles of the week ───────────────────────────────────────────
function getWeeklyTopArticles(limit = 6): Array<{
  id: string; titulo: string; bajada: string; slug: string;
  categoria: string; views: number; published_at: string;
}> {
  return sqlite.prepare(`
...
async function generateDigestIntro(articles: ReturnType<typeof getWeeklyTopArticles>): Promise<string> {
  const articleList = articles.map((a, i) =>
    `${i + 1}. [${a.categoria.toUpperCase()}] ${a.titulo} — ${a.bajada.substring(0, 100)}`
  ).join('\n');
...
function buildEmailHTML(
  intro: string,
  articles: ReturnType<typeof getWeeklyTopArticles>,
  unsubToken: string,
  ...
  const introHTML = intro.split('\n\n').map(p =>
    `<p style="margin:0 0 14px; font-family:Georgia,'Times New Roman',serif;
      font-size:15px; color:#333; line-height:1.75;">${p}</p>`
  ).join('');
...
export function getNewsletterStats() {
  return {
    active:       (sqlite.prepare("SELECT COUNT(*) as c FROM newsletter_subscribers WHERE status='active' AND confirmed=1").get() as {c:number}).c,
    unconfirmed:  (sqlite.prepare("SELECT COUNT(*) as c FROM newsletter_subscribers WHERE confirmed=0").get() as {c:number}).c,
    unsubscribed: (sqlite.prepare("SELECT COUNT(*) as c FROM newsletter_subscribers WHERE status='unsubscribed'").get() as {c:number}).c,
    lastSend:     sqlite.prepare("SELECT subject, sent_at, recipient_count FROM newsletter_sends ORDER BY sent_at DESC LIMIT 1").get(),
    totalSends:   (sqlite.prepare("SELECT COUNT(*) as c FROM newsletter_sends WHERE status='sent'").get() as {c:number}).c,
  };
}
