...
import { MODELS, estimateCost } from '../config/models.js';
...
export async function extractPlayerMentions(
  articleText: string
...
Si no hay ninguno: {"players": []}`,
    ...
    const parsed = JSON.parse(clean) as { players: string[] };
...
export async function generatePlayerStub(
  playerName: string,
  articleContext?: string  // the article where they were first mentioned
): Promise<string | null> {
  try {
...
Si no conocés al jugador con certeza, dejá los campos en null. NO inventés datos.`;
    ...
    const existing = sqlite.prepare('SELECT id FROM players WHERE slug = ?').get(slug) as { id: string } | undefined;
...
export async function linkArticleToPlayers(
  articleId: string,
  articleTitle: string,
  articleText: string
): Promise<string[]> {
  ...
  for (const { name, slug } of mentions) {
    ...
    let player = sqlite.prepare('SELECT id, article_mentions FROM players WHERE slug = ?').get(slug) as
      { id: string; article_mentions: string } | undefined;
    ...
    if (!player) {
      ...
      if (newId) {
        player = sqlite.prepare('SELECT id, article_mentions FROM players WHERE id = ?')
          .get(newId) as { id: string; article_mentions: string };
      }
    }

    if (player) {
      // Add article to mentions list
      const mentions = JSON.parse(player.article_mentions || '[]') as string[];
...
export async function handleTransfer(data: {
  ...
  note: string;             // e.g. "Préstamo del Atlético de Madrid"
  articleId?: string;
}): Promise<void> {
  const slug = toSlug(data.playerName);
  const player = sqlite.prepare('SELECT id FROM players WHERE slug = ?').get(slug) as { id: string } | undefined;
...
export async function enrichPlayerProfile(playerId: string): Promise<void> {
  const player = sqlite.prepare('SELECT * FROM players WHERE id = ? AND enriched = 0').get(playerId) as
    Record<string, unknown> | undefined;

  if (!player) return;

  const mentions = JSON.parse(player.article_mentions as string || '[]') as string[];
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Enriquecé el perfil de ${player.name as string}, jugador de River Plate, para desdeeltablon.com.
Bio actual: "${player.bio as string || 'Sin bio'}"
...
NO inventés datos. Si no conocés al jugador en profundidad, mejorá solo lo que podés confirmar.`
    ...
    const data = JSON.parse(clean) as {
      ...
      comparison?: string;
      histNote?: string;
      additionalStats?: Array<[string, string]>;
    };

    const currentStats = JSON.parse(player.stats_json as string || '[]') as Array<[string, string]>;
    ...
    console.log(`✨ Perfil enriquecido: ${player.name as string}`);
  } catch (err) {
    console.error(`Error enriqueciendo perfil ${player.name as string}:`, err);
  }
}

// ─── Get player profile for API ──────────────────────────────────────────────
export function getPlayerBySlug(slug: string) {
  const player = sqlite.prepare('SELECT * FROM players WHERE slug = ?').get(slug) as
    Record<string, unknown> | undefined;

  if (!player) return null;

  // Get related articles
  const articleIds = JSON.parse(player.article_mentions as string || '[]') as string[];
  const articles = articleIds.length > 0
    ? sqlite.prepare(`
      ...
      `).all(...articleIds) as Array<Record<string, unknown>>
    : [];

  return {
    ...player,
    stats: JSON.parse(player.stats_json as string || '[]'),
    titles: JSON.parse(player.titles_json as string || '[]'),
    articles,
  };
}
