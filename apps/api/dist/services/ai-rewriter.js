    ...
    return articles.map(a => {
        ...
        if (a.structuredFacts) {
            ...
            if ('homeScore' in f) {
                ...
                if (m.venue)
                    lines.push(`Estadio: ${m.venue}`);
                if (m.goals?.length)
                    lines.push(`Goles: ${m.goals.map(g => `${g.player} ${g.minute}' (${g.type})`).join(', ')}`);
                if (m.stats)
                    lines.push(`Stats: ${Object.entries(m.stats).map(([k, v]) => `${k}: ${v}`).join(' | ')}`);
                if (m.keyMoments?.length)
                    lines.push(`Momentos: ${m.keyMoments.join(' | ')}`);
            }
                ...
                if (t.fee)
                    lines.push(`Monto: ${t.fee}`);
                if (t.contractUntil)
                    lines.push(`Contrato hasta: ${t.contractUntil}`);
...
async function callWriter(sourcesText, extraInstructions, categoria = 'actualidad') {
...
async function callVerifier(article, sourcesText) {
...
export async function rewriteArticle(rawArticles, additionalContext) {
...
function logDiscard(data) {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(uuidv4(), data.originalTitle, data.reason, data.writerAttempts, JSON.stringify(data.datosProblema), JSON.stringify(data.problemasEditoriales));
    }
    catch { /* non-critical */ }
}
// ─── Historical note ──────────────────────────────────────────────────────────
export async function generateHistoricalNote(dayMonth, historicalFact) {
    const sources = [{
...
// ─── Tactical analysis ────────────────────────────────────────────────────────
export async function generateTacticalAnalysis(matchFacts) {
    const sources = [{
...
// ─── Transfer article ─────────────────────────────────────────────────────────
export async function generateTransferArticle(transferFacts, additionalSources) {
    const primary = {
        source: transferFacts.sources[0] || 'fuente especializada',
        level: transferFacts.confirmationLevel,
        url: '',
