...
function resolveVenue(isHome, opponentName) {
    if (isHome)
        return 'Estadio Más Monumental';
...
function extractNextData(html) {
    ...
    if (start === -1)
        throw new Error('__NEXT_DATA__ no encontrado');
...
async function scrapeStandingsPage(url, competitionLabel, mode) {
    ...
    if (allBlocks.length === 0)
        throw new Error('Sin equipos en ' + url);
...
async function scrapePromiedosStandings() {
...
async function scrapeCopaStandings() {
    ...
    for (const [url, name] of [
        ...
        try {
            ...
            console.log(`⚠️ ${name} no disponible:`, e.message);
        }
    }
    throw new Error('River no está en ninguna Copa CONMEBOL actualmente');
}
// ─── Tabla Anual ──────────────────────────────────────────────────────────────
async function scrapeAnualStandings() {
...
async function scrapePromiedosMatches() {
    ...
    if (!games)
        throw new Error('No se encontró games en __NEXT_DATA__');
    function buildMatch(row, isHomeOverride) {
        const game = row.game;
        if (!game)
            return null;
    ...
    let lastMatch = null;
    const lastRows = (games.last?.rows || []);
    // Filtrar solo Liga Profesional y ordenar por fecha descendente
    const playedRows = lastRows.filter((row) => {
        const game = row.game;
...
export async function getStandings() {
    ...
    try {
        ...
        console.log('⚠️ Promiedos tabla falló:', err.message);
    }
...
export async function getNextMatch() {
    ...
    try {
        ...
        console.log('⚠️ Próximo partido falló:', err.message);
    }
...
export async function getLastResult() {
    ...
    try {
        ...
        console.log('⚠️ Último resultado falló:', err.message);
    }
    ...
    try {
        ...
        function buildRow(row) {
            ...
            if (!game)
                return null;
            const vals = row.values;
            const homeAway = (vals.find((v) => v.key === 'home_away')?.value) || 'L';
        ...
        const upcoming = (games.next?.rows || []).map(buildRow).filter(Boolean);
        const resultRows = [...(games.last?.rows || [])].filter((r) => r.game?.winner !== -1 && r.game?.winner !== undefined);
        resultRows.sort((a, b) => {
            const parseDate = (r) => {
                try {
        ...
        const results = resultRows.map(buildRow).filter(Boolean);
...
export async function getZonaStandings() {
    ...
    try {
        ...
        console.log('⚠️ Zona standings falló:', err.message);
...
export async function getAnualStandings() {
    ...
    try {
        ...
        console.log('⚠️ Anual standings falló:', err.message);
