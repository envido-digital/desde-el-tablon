...
import { rewriteArticle, generateHistoricalNote, initDiscardLog } from '../services/ai-rewriter.js';
...
export function saveArticle(data) {
    ...
    INSERT INTO articles (
      ...
      CURRENT_TIMESTAMP, 0)
  `).run(id, data.titulo, data.bajada, data.cuerpo, slug, data.metaDescription, data.categoria, JSON.stringify(data.tags), JSON.stringify(data.keywords), data.importanceScore || 0.5, data.tiempoLectura, data.featuredImage ? JSON.stringify(data.featuredImage) : null, JSON.stringify({
...
function calcImportance(sourceLevel, itemCount) {
...
export async function runPipeline(options = {}) {
    ...
    try {
        ...
        for (const group of groups) {
            try {
                const rawArticles = group.items.map(item => ({
                    source: item.source,
                    level: group.sourceLevel,
                    title: item.title,
...
export async function generateDailyHistoricalNote() {
    const today = new Date();
    const todayStr = today.toISOString().split('T')[0];
    if (sqlite.prepare("SELECT id FROM articles WHERE categoria='historia' AND date(published_at)=? AND status='published'").get(todayStr))
        return null;
...
// ─── Match analysis ───────────────────────────────────────────────────────────
export async function generateMatchAnalysis(matchFacts) {
...
// ─── Transfer news ────────────────────────────────────────────────────────────
export async function generateTransferNews(transferFacts) {
