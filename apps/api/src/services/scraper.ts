...
export function saveScrapedItem(item: ScrapedItem & { sourceId: string }): string | null {
...
export async function scrapeRSS(sourceId: string, rssUrl: string): Promise<ScrapedItem[]> {
  try {
    ...
    for (const item of (feed.items || []).slice(0, 20)) {
      ...
      if (relevance < 0.3) continue; // Skip irrelevant items
...
export async function scrapeAllSources(): Promise<ScrapedItem[]> {
  const sources = sqlite.prepare('SELECT * FROM news_sources WHERE active = 1 ORDER BY level ASC').all() as Array<{
    id: string;
    name: string;
    url: string;
    level: number;
...
export function getUnprocessedItems(limit = 10): Array<{ items: ScrapedItem[]; sourceLevel: number }> {
  const raw = sqlite.prepare(`
    ...
    LIMIT ?
  `).all(limit) as Array<{ raw_data: string; source_level: number; id: string }>;

  // Group similar items (same event, multiple sources)
  const grouped: Map<string, { items: ScrapedItem[]; sourceLevel: number; ids: string[] }> = new Map();

  for (const row of raw) {
    try {
      const item = JSON.parse(row.raw_data) as ScrapedItem;
...
export function markItemsProcessed(articleId: string, titleHashes: string[]) {
