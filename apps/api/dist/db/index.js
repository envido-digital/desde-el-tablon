...
export function initDb() {
  ...
  `);
    const sourceCount = sqlite.prepare('SELECT COUNT(*) as count FROM news_sources').get().count;
    if (sourceCount === 0) {
    ...
    const badgeCount = sqlite.prepare('SELECT COUNT(*) as count FROM badges').get().count;
    if (badgeCount === 0) {
    ...
    if (adminEmail && adminPass) {
        ...
        if (!existing) {
            ...
            import('../lib/auth.js').then(async ({ hashPassword }) => {
                ...
                sqlite.prepare("INSERT OR IGNORE INTO users (id,email,username,password_hash,role,email_verified,total_points) VALUES (?,?,?,?,'admin',1,0)").run(uuidv4(), adminEmail, 'admin', hash);
