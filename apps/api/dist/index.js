...
app.use('/api/sports', (req, res, next) => {
    if (req.method === 'GET')
        setCacheHeaders(res, 300); // sports data: 5 min
    next();
}, sportsRouter);
app.use('/api/logo', (req, res, next) => {
    if (req.method === 'GET')
        setCacheHeaders(res, 2592000); // logos: 30 days
    next();
}, logoRouter);
...
app.get('/sitemap.xml', (_req, res) => {
    const articles = sqlite.prepare("SELECT slug, published_at FROM articles WHERE status='published' ORDER BY published_at DESC LIMIT 1000").all();
    const base = process.env.SITE_URL || 'https://desdeeltablon.com';
    const players = sqlite.prepare("SELECT slug, updated_at FROM players WHERE status='active' ORDER BY updated_at DESC").all();
    const urls = [
...
╚════════════════════════════════════════════════╝
    ...
    const count = sqlite.prepare('SELECT COUNT(*) as c FROM articles').get().c;
    if (count === 0) {
...
function seedDemoArticles() {
  ...
  `);
    ...
    for (const d of demos) {
        ins.run(d.id, d.titulo, d.bajada, d.cuerpo, d.slug, d.meta, d.categoria, d.tags, d.keywords, d.score, d.reading, d.offset, d.views);
    }
