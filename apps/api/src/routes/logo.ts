...
logoRouter.get("/:teamName", async (req: Request, res: Response) => {
...
// Body: { teams: string[] }
logoRouter.post("/batch", async (req: Request, res: Response) => {
...
logoRouter.get("/_stats", (_req: Request, res: Response) => {
  res.json(getLogoCacheStats());
});

// DELETE /api/logo/_cache — clear logo cache (admin)
logoRouter.delete("/_cache", (req: Request, res: Response) => {
  const { team } = req.query;
  clearLogoCache(team as string | undefined);
