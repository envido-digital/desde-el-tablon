...
async function searchWikimediaCommons(teamName) {
...
export async function resolveLogo(teamName) {
    if (!teamName?.trim())
        return null;
    // 1. Check cache
    const cached = sqlite.prepare("SELECT data_url, source FROM logo_cache WHERE team_name = ?").get(teamName);
    if (cached) {
...
export function clearLogoCache(teamName) {
    if (teamName) {
...
export function getLogoCacheStats() {
    return {
        total: sqlite.prepare("SELECT COUNT(*) as c FROM logo_cache").get().c,
        found: sqlite.prepare("SELECT COUNT(*) as c FROM logo_cache WHERE data_url IS NOT NULL").get().c,
        notFound: sqlite.prepare("SELECT COUNT(*) as c FROM logo_cache WHERE data_url IS NULL").get().c,
    };
}
