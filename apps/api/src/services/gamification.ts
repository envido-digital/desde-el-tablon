...
export function awardPoints(userId: string, action: keyof typeof POINT_ACTIONS, articleId?: string): {
  points: number;
  newTotal: number;
  levelUp: boolean;
  newLevel?: (typeof LEVELS)[0];
} | null {
  const user = sqlite.prepare('SELECT * FROM users WHERE id = ?').get(userId) as {
    id: string; total_points: number; level: number;
  } | undefined;
...
export function hasReadArticleToday(userId: string, articleId: string): boolean {
...
export function checkAndAwardBadges(userId: string, totalPoints: number) {
  const availableBadges = sqlite.prepare(`
    SELECT * FROM badges WHERE points_required <= ? AND points_required > 0
  `).all(totalPoints) as Array<{ id: string; points_required: number }>;
...
// Record daily login
export function recordDailyLogin(userId: string): { alreadyLoggedIn: boolean; points?: number } {
  ...
  if (lastLogin) return { alreadyLoggedIn: true };
